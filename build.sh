...
# Get build version
MAJOR=$(cat $DIR/vendor/hive/vendor.mk | grep 'ROM_VERSION_MAJOR := *' | sed  's/ROM_VERSION_MAJOR := //g')
MINOR=$(cat $DIR/vendor/hive/vendor.mk | grep 'ROM_VERSION_MINOR := *' | sed  's/ROM_VERSION_MINOR := //g')
MAINTENANCE=$(cat $DIR/vendor/hive/vendor.mk | grep 'ROM_VERSION_MAINTENANCE := *' | sed  's/ROM_VERSION_MAINTENANCE := //g')
TAG=$(cat $DIR/vendor/hive/vendor.mk | grep 'ROM_VERSION_TAG := *' | sed  's/ROM_VERSION_TAG := //g')
...
echo -e "${cya}Building ${bldcya}HIVE $VERSION for $DEVICE ${txtrst}";
...
if [ -n "${INTERACTIVE}" ]; then
        ...
        echo -en "${bldblu}Remember to lunch you device:"
        if [ "${VENDOR}" == "hive" ]; then
                echo -e "[${bldgrn}lunch hive_$DEVICE-userdebug${bldblu}]${txtrst}"
        else
